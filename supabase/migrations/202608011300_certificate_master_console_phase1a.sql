...
--
-- Multi-institution and multi-category master-template administration.
-- The dedicated `certificate_master_*` namespace deliberately avoids the
-- pre-existing `agilecert_certificate_templates` authority table.
-- ---------------------------------------------------------------------------
...
create table if not exists public.agilecert_certificate_role_permissions (
  ...
  permission_key text not null
    references public.agilecert_certificate_permission_definitions(permission_key)
    on delete cascade,
...
insert into public.agilecert_certificate_categories (
  ...
  requires_score, sort_order, is_active
) values
...
create table if not exists public.agilecert_certificate_assets (
  ...
  institution_id uuid not null
    references public.agilecert_certificate_institutions(id) on delete restrict,
...
create unique index if not exists agilecert_certificate_assets_version_uidx
  on public.agilecert_certificate_assets(
    institution_id, asset_type, lower(name), version_number
  );
...
create table if not exists public.agilecert_certificate_master_templates (
  id uuid primary key default gen_random_uuid(),
  institution_id uuid not null
    references public.agilecert_certificate_institutions(id) on delete restrict,
  category_id uuid not null
    references public.agilecert_certificate_categories(id) on delete restrict,
  code text not null,
  name text not null,
  description text not null default '',
  ...
  page_size text not null default 'A4'
    check (page_size in ('A4', 'Letter', 'Legal', 'Custom')),
  status text not null default 'draft'
    check (status in ('draft', 'in_review', 'approved', 'published', 'retired')),
  current_version_id uuid,
  required_fields jsonb not null default
    '["holderName","certificateTitle","programmeTitle","issueDate","certificateNumber","verificationCode","qrCode"]'::jsonb,
  quality_standard jsonb not null default
    '{"minimumPrintDpi":300,"masterFormats":["pdf","svg","png","jpeg"],"singlePageRequired":true,"physicalPrintReviewRequired":true,"longNameTestRequired":true,"qrScanTestRequired":true}'::jsonb,
...
create unique index if not exists agilecert_certificate_master_templates_code_uidx
  on public.agilecert_certificate_master_templates(institution_id, upper(code));
create index if not exists agilecert_certificate_master_templates_category_idx
  on public.agilecert_certificate_master_templates(category_id, status, institution_id);

create table if not exists public.agilecert_certificate_master_versions (
  id uuid primary key default gen_random_uuid(),
...
alter table public.agilecert_certificate_master_templates
  drop constraint if exists agilecert_certificate_master_templates_current_version_fk;
alter table public.agilecert_certificate_master_templates
  add constraint agilecert_certificate_master_templates_current_version_fk
  foreign key (current_version_id)
  references public.agilecert_certificate_master_versions(id)
  on delete set null;

create index if not exists agilecert_certificate_master_versions_template_idx
  on public.agilecert_certificate_master_versions(template_id, version_number desc);
create index if not exists agilecert_certificate_master_versions_status_idx
  on public.agilecert_certificate_master_versions(status, quality_status, created_at desc);

create table if not exists public.agilecert_certificate_master_assignments (
  id uuid primary key default gen_random_uuid(),
  category_id uuid not null
    references public.agilecert_certificate_categories(id) on delete restrict,
  template_id uuid not null
...
create index if not exists agilecert_certificate_master_assignments_resolution_idx
  on public.agilecert_certificate_master_assignments(
    category_id, scope_type, is_active, priority, effective_from desc
  );
create unique index if not exists agilecert_certificate_master_assignments_global_active_uidx
  on public.agilecert_certificate_master_assignments(category_id)
  where scope_type = 'global' and is_active;
create unique index if not exists agilecert_certificate_master_assignments_programme_active_uidx
  on public.agilecert_certificate_master_assignments(category_id, programme_id)
  where scope_type = 'programme' and is_active;
create unique index if not exists agilecert_certificate_master_assignments_exam_active_uidx
  on public.agilecert_certificate_master_assignments(category_id, examination_id)
  where scope_type = 'examination' and is_active;

create table if not exists public.agilecert_certificate_master_audit (
...
create index if not exists agilecert_certificate_master_audit_entity_idx
  on public.agilecert_certificate_master_audit(entity_type, entity_id, created_at desc);
create index if not exists agilecert_certificate_master_audit_actor_idx
  on public.agilecert_certificate_master_audit(actor_id, created_at desc);

-- Timestamp maintenance.
drop trigger if exists agilecert_certificate_permission_definitions_updated_at
  on public.agilecert_certificate_permission_definitions;
...
drop trigger if exists agilecert_certificate_institutions_updated_at
  on public.agilecert_certificate_institutions;
create trigger agilecert_certificate_institutions_updated_at
  before update on public.agilecert_certificate_institutions
  for each row execute function public.set_updated_at();

drop trigger if exists agilecert_certificate_categories_updated_at
  on public.agilecert_certificate_categories;
create trigger agilecert_certificate_categories_updated_at
  before update on public.agilecert_certificate_categories
  for each row execute function public.set_updated_at();

drop trigger if exists agilecert_certificate_assets_updated_at
  on public.agilecert_certificate_assets;
create trigger agilecert_certificate_assets_updated_at
  before update on public.agilecert_certificate_assets
  for each row execute function public.set_updated_at();

drop trigger if exists agilecert_certificate_master_templates_updated_at
  on public.agilecert_certificate_master_templates;
create trigger agilecert_certificate_master_templates_updated_at
  before update on public.agilecert_certificate_master_templates
  for each row execute function public.set_updated_at();

drop trigger if exists agilecert_certificate_master_versions_updated_at
  on public.agilecert_certificate_master_versions;
create trigger agilecert_certificate_master_versions_updated_at
  before update on public.agilecert_certificate_master_versions
  for each row execute function public.set_updated_at();

drop trigger if exists agilecert_certificate_master_assignments_updated_at
  on public.agilecert_certificate_master_assignments;
create trigger agilecert_certificate_master_assignments_updated_at
  before update on public.agilecert_certificate_master_assignments
  for each row execute function public.set_updated_at();
...
alter table public.agilecert_certificate_master_templates enable row level security;
alter table public.agilecert_certificate_master_versions enable row level security;
alter table public.agilecert_certificate_master_assignments enable row level security;
alter table public.agilecert_certificate_master_audit enable row level security;
...
revoke all on table public.agilecert_certificate_master_templates from public, anon, authenticated;
revoke all on table public.agilecert_certificate_master_versions from public, anon, authenticated;
revoke all on table public.agilecert_certificate_master_assignments from public, anon, authenticated;
revoke all on table public.agilecert_certificate_master_audit from public, anon, authenticated;
revoke all on sequence public.agilecert_certificate_master_audit_id_seq from public, anon, authenticated;

create or replace function public.agilecert_certificate_has_permission(
...
revoke all on function public.agilecert_certificate_has_permission(text)
  from public, anon, authenticated;
grant execute on function public.agilecert_certificate_has_permission(text)
  to authenticated;
...
revoke all on function public.agilecert_certificate_require_permission(text)
  from public, anon, authenticated;
...
as $$
begin
  insert into public.agilecert_certificate_master_audit (
...
revoke all on function public.agilecert_certificate_write_audit(
  text, text, text, jsonb, jsonb, jsonb
) from public, anon, authenticated;

create or replace function public.prevent_agilecert_certificate_master_audit_mutation()
returns trigger
language plpgsql
security definer
set search_path = public
as $$
begin
  raise exception 'Certificate master audit records are immutable.';
end;
$$;

revoke all on function public.prevent_agilecert_certificate_master_audit_mutation()
  from public, anon, authenticated;

drop trigger if exists agilecert_certificate_master_audit_immutable
  on public.agilecert_certificate_master_audit;
create trigger agilecert_certificate_master_audit_immutable
  before update or delete on public.agilecert_certificate_master_audit
  for each row execute function public.prevent_agilecert_certificate_master_audit_mutation();
...
begin
  ...
  select coalesce(
    jsonb_agg(definition.permission_key order by definition.permission_key),
    '[]'::jsonb
  )
  into v_permissions
...
revoke all on function public.get_my_certificate_management_access()
  from public, anon, authenticated;
grant execute on function public.get_my_certificate_management_access()
  to authenticated;

create or replace function public.get_certificate_management_console_snapshot(
...
begin
  ...
  return jsonb_build_object(
    ...
    'summary', jsonb_build_object(
      ...
      'versionsAwaitingReview', (
        select count(*) from public.agilecert_certificate_master_versions
        where status = 'in_review'
      ),
      'approvedAssets', (
        select count(*) from public.agilecert_certificate_assets
    ...
    'templates', coalesce((
      ...
      from (
        ...
        left join public.agilecert_certificate_master_versions current_version
...
revoke all on function public.get_certificate_management_console_snapshot(integer)
  from public, anon, authenticated;
grant execute on function public.get_certificate_management_console_snapshot(integer)
  to authenticated;
...
begin
  ...
  if v_code !~ '^[A-Z0-9_-]{2,30}$' then
    raise exception 'Enter a valid institution code.';
  end if;
  if length(v_name) < 3 then
    raise exception 'Enter the institution name.';
  end if;

  if p_id is null then
    insert into public.agilecert_certificate_institutions (
      code, name, short_name, legal_name, registration_details,
      country_code, website, contact_email, is_active,
      created_by, updated_by
    ) values (
      v_code, v_name, nullif(trim(p_short_name), ''),
      nullif(trim(p_legal_name), ''),
      nullif(trim(p_registration_details), ''),
      upper(trim(coalesce(p_country_code, 'NG'))),
      nullif(trim(p_website), ''),
      nullif(lower(trim(p_contact_email)), ''),
      coalesce(p_is_active, true), auth.uid(), auth.uid()
    ) returning * into v_record;
  else
    select to_jsonb(current_record)
    into v_before
...
begin
  ...
  if v_code !~ '^[a-z0-9_-]{2,40}$' then
    raise exception 'Enter a valid category code.';
  end if;
  if length(v_name) < 3 then
  ...
  if p_id is null then
    insert into public.agilecert_certificate_categories (
      ...
      coalesce(p_requires_identity_verification, false),
      coalesce(p_requires_score, false),
      greatest(1, coalesce(p_sort_order, 100)),
      coalesce(p_is_active, true), auth.uid(), auth.uid()
    ) returning * into v_record;
  else
    select to_jsonb(current_record)
    ...
    update public.agilecert_certificate_categories set
      ...
      requires_identity_verification = coalesce(
        p_requires_identity_verification, false
      ),
    ...
    if v_record.id is null then
      raise exception 'Certificate category not found.';
    end if;
  end if;
...
as $$
declare
  v_record public.agilecert_certificate_master_templates;
  v_code text := upper(trim(coalesce(p_code, '')));
  v_name text := trim(coalesce(p_name, ''));
begin
  perform public.agilecert_certificate_require_permission(
  ...
  if v_code !~ '^[A-Z0-9_-]{2,50}$' then
    raise exception 'Enter a valid template code.';
  end if;
  if length(v_name) < 3 then
    raise exception 'Enter the template name.';
  end if;
  if p_orientation not in ('portrait', 'landscape') then
    raise exception 'Invalid certificate orientation.';
  end if;
  if p_page_size not in ('A4', 'Letter', 'Legal', 'Custom') then
    raise exception 'Invalid certificate page size.';
  end if;
  if not exists (
    select 1 from public.agilecert_certificate_institutions
    where id = p_institution_id and is_active
  ) then
    raise exception 'Select an active issuing institution.';
  end if;
  if not exists (
    select 1 from public.agilecert_certificate_categories
  ...
  insert into public.agilecert_certificate_master_templates (
    ...
    p_institution_id, p_category_id, v_code, v_name,
    trim(coalesce(p_description, '')), p_orientation, p_page_size,
    nullif(trim(p_notes), ''), auth.uid(), auth.uid()
  ) returning * into v_record;

  perform public.agilecert_certificate_write_audit(
    'template', v_record.id::text, 'template.created',
    null, to_jsonb(v_record), '{}'::jsonb
  );
...
declare
  v_version integer;
  v_record public.agilecert_certificate_master_versions;
begin
  perform public.agilecert_certificate_require_permission(
    'certificate.templates.manage'
  );

  if p_storage_bucket <> 'certificate-masters' then
    raise exception 'Invalid certificate master bucket.';
  end if;
  if p_source_format not in ('pdf', 'svg', 'png', 'jpeg') then
    raise exception 'Unsupported master-template format.';
  end if;
  if p_file_size_bytes is null or p_file_size_bytes <= 0 then
    raise exception 'The uploaded master file is empty.';
  end if;
  if not exists (
  ...
  insert into public.agilecert_certificate_master_versions (
    template_id, version_number, source_format,
    storage_bucket, storage_path, original_filename, mime_type,
    file_size_bytes, sha256, pixel_width, pixel_height,
    notes, created_by
  ) values (
    p_template_id, v_version, p_source_format,
    p_storage_bucket, trim(p_storage_path), trim(p_original_filename),
    trim(p_mime_type), p_file_size_bytes,
    nullif(lower(trim(p_sha256)), ''),
  ...
  perform public.agilecert_certificate_write_audit(
    'template_version', v_record.id::text,
    'template_version.registered', null, to_jsonb(v_record),
...
begin
  ...
  if p_storage_bucket <> 'certificate-assets' then
    raise exception 'Invalid certificate asset bucket.';
  end if;
  if p_asset_type not in (
    'logo', 'seal', 'signature', 'watermark',
    'background', 'emblem', 'other'
  ) then
    raise exception 'Unsupported certificate asset type.';
  end if;
  if length(v_name) < 2 then
    raise exception 'Enter an asset name.';
  end if;
  if p_file_size_bytes is null or p_file_size_bytes <= 0 then
    raise exception 'The uploaded asset is empty.';
  end if;
  if not exists (
    select 1 from public.agilecert_certificate_institutions
  ...
  perform public.agilecert_certificate_write_audit(
    'asset', v_record.id::text, 'asset.registered',
    null, to_jsonb(v_record), '{}'::jsonb
  );
...
begin
  ...
  if v_record.id is null then
    raise exception 'Master template version not found.';
  end if;

  perform public.agilecert_certificate_write_audit(
    'template_version', v_record.id::text,
    'template_version.quality_reviewed',
    v_before, to_jsonb(v_record),
    jsonb_build_object('qualityStatus', p_quality_status)
  );
...
declare
  v_before jsonb;
  v_record public.agilecert_certificate_master_versions;
  v_template public.agilecert_certificate_master_templates;
begin
  select *
  into v_record
  from public.agilecert_certificate_master_versions
  ...
  if p_action = 'submit_review' then
    ...
    if v_record.status not in ('draft', 'changes_requested', 'rejected') then
      raise exception 'Only draft or returned versions may be submitted.';
    end if;
    ...
    update public.agilecert_certificate_master_templates set
      status = 'in_review', updated_by = auth.uid()
    ...
    if v_record.status <> 'in_review' then
      raise exception 'Only versions in review may be returned.';
    end if;
    ...
    if v_record.status <> 'in_review' then
      raise exception 'Only versions in review may be rejected.';
    end if;
    ...
    if v_record.status <> 'in_review' then
      raise exception 'Only versions in review may be approved.';
    end if;
    ...
    update public.agilecert_certificate_master_templates set
      status = 'approved', updated_by = auth.uid()
    where id = v_template.id
      and status <> 'published';
    ...
    if v_record.status <> 'approved' then
      raise exception 'Only approved versions may be published.';
    end if;
    if not exists (
      select 1
      from public.agilecert_certificate_master_templates template
      ...
      raise exception 'The institution and certificate category must be active.';
    end if;

    update public.agilecert_certificate_master_versions
  ...
  perform public.agilecert_certificate_write_audit(
    'template_version', v_record.id::text,
    'template_version.' || p_action,
    v_before, to_jsonb(v_record),
    jsonb_build_object('templateId', v_template.id)
  );
...
begin
  ...
  update public.agilecert_certificate_assets set
    ...
    retired_by = case
      when p_status = 'retired' then auth.uid()
      else retired_by
    end,
    retired_at = case
      when p_status = 'retired' then now()
      else retired_at
    end
  where id = p_asset_id
  returning * into v_record;
  ...
  if v_record.id is null then
    raise exception 'Certificate asset not found.';
  end if;
...
declare
  ...
  v_record public.agilecert_certificate_master_assignments;
begin
  perform public.agilecert_certificate_require_permission(
  ...
  if v_template.id is null
    ...
    or v_version.template_id <> v_template.id then
    raise exception 'The master template and version do not match.';
  end if;
  if v_version.status <> 'published' then
    raise exception 'Only a published master version may be assigned.';
  end if;
  if p_scope_type not in ('global', 'programme', 'examination') then
    raise exception 'Invalid assignment scope.';
  end if;
  ...
  if p_scope_type = 'global' then
    ...
    if p_programme_id is null or not exists (
      select 1 from public.programmes where id = p_programme_id
    ) then
    ...
    if p_examination_id is null then
      raise exception 'Select a valid examination.';
    end if;
    select examination.programme_id
    into v_exam_programme
    from public.examinations examination
    where examination.id = p_examination_id;

    if v_exam_programme is null then
      raise exception 'Select a valid examination.';
    end if;
  ...
  insert into public.agilecert_certificate_master_assignments (
    category_id, template_id, template_version_id, scope_type,
    programme_id, examination_id, priority,
    effective_from, effective_to, is_active,
    created_by, updated_by
  ) values (
    v_template.category_id, v_template.id, v_version.id, p_scope_type,
    p_programme_id, p_examination_id,
    greatest(1, least(coalesce(p_priority, 100), 1000)),
    coalesce(p_effective_from, now()), p_effective_to,
    true, auth.uid(), auth.uid()
  ) returning * into v_record;

  perform public.agilecert_certificate_write_audit(
    'assignment', v_record.id::text, 'assignment.created',
    null, to_jsonb(v_record),
    jsonb_build_object(
...
begin
  ...
  if length(trim(coalesce(p_reason, ''))) < 5 then
    raise exception 'Enter an assignment change reason.';
  end if;
  ...
  update public.agilecert_certificate_master_assignments set
    ...
    effective_to = case
      when coalesce(p_is_active, false) then null
      else coalesce(effective_to, now())
    end,
    updated_by = auth.uid()
  ...
  if v_record.id is null then
    raise exception 'Master template assignment not found.';
  end if;

  perform public.agilecert_certificate_write_audit(
    'assignment', v_record.id::text,
    case
      when v_record.is_active then 'assignment.activated'
      else 'assignment.deactivated'
    end,
    v_before, to_jsonb(v_record),
...
begin
  ...
  if length(trim(coalesce(p_reason, ''))) < 5 then
    raise exception 'Enter a permission change reason.';
  end if;
  if not exists (
    select 1
    ...
    raise exception 'Certificate permission not found.';
  end if;
  if p_permission_key = 'certificate.permissions.manage'
    and coalesce(p_is_granted, false) then
    raise exception 'Examination Administrators cannot receive permission-management authority.';
  end if;

  select to_jsonb(permission)
  into v_before
  from public.agilecert_certificate_role_permissions permission
  where permission.role = 'exam_admin'
    and permission.permission_key = p_permission_key;

  insert into public.agilecert_certificate_role_permissions (
    role, permission_key, is_granted, updated_by, updated_at
  ) values (
    'exam_admin', p_permission_key,
    coalesce(p_is_granted, false), auth.uid(), now()
  )
  ...
  perform public.agilecert_certificate_write_audit(
    'permission', p_permission_key, 'permission.updated',
    v_before, to_jsonb(v_record),
    jsonb_build_object(
...
-- Future server-side renderer resolver. Browser roles deliberately receive no
-- execute grant in Phase 1A.
...
revoke all on function public.resolve_agilecert_certificate_master(
  uuid, text, timestamptz
) from public, anon, authenticated;

-- Private immutable source-file buckets.
...
-- Browser writes are permission-checked RPCs only.
grant execute on function public.certificate_admin_upsert_institution(
  uuid, text, text, text, text, text, text, text, text, boolean
) to authenticated;
grant execute on function public.certificate_admin_upsert_category(
  uuid, text, text, text, boolean, boolean, integer, boolean
) to authenticated;
grant execute on function public.certificate_admin_create_template(
  uuid, uuid, text, text, text, text, text, text
) to authenticated;
grant execute on function public.certificate_admin_register_template_version(
  uuid, text, text, text, text, text, bigint, text, integer, integer, text
) to authenticated;
grant execute on function public.certificate_admin_register_asset(
  uuid, text, text, text, text, text, text, bigint, text, integer, integer
) to authenticated;
grant execute on function public.certificate_admin_record_quality_review(
  uuid, text, jsonb, text
) to authenticated;
grant execute on function public.certificate_admin_transition_template_version(
  uuid, text, text
) to authenticated;
grant execute on function public.certificate_admin_set_asset_status(
  uuid, text, text
) to authenticated;
grant execute on function public.certificate_admin_assign_template(
  uuid, uuid, text, uuid, uuid, integer, timestamptz, timestamptz
) to authenticated;
grant execute on function public.certificate_admin_set_assignment_active(
  uuid, boolean, text
) to authenticated;
grant execute on function public.certificate_admin_set_permission(
  text, boolean, text
) to authenticated;

comment on table public.agilecert_certificate_master_templates is
  'Institution- and category-specific certificate master definitions. Existing issued certificates and the legacy certificate template authority remain unchanged.';
comment on table public.agilecert_certificate_master_versions is
  'Immutable uploaded certificate master versions with print-quality review and publication workflow.';
comment on table public.agilecert_certificate_master_assignments is
  'Published master assignments resolved by examination, programme or global certificate category scope.';
comment on function public.resolve_agilecert_certificate_master(uuid,text,timestamptz) is
  'Returns the highest-specificity published master for future controlled server-side rendering.';
