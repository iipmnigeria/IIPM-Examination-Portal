...
-- Production recorded the legacy Commerce Console migration, but its RPCs are
-- absent. Restore the owner-only compatibility layer required by the
-- permission-scoped Finance Console wrappers introduced in the next migration.
...
declare
  ...
  v_result jsonb;
begin
  if coalesce(public.current_user_role(), '') not in ('exam_admin', 'super_admin') then
...
begin
  ...
  if v_currency !~ '^[A-Z]{3}$' then raise exception 'Currency must be a valid three-letter ISO code.'; end if;
  if p_amount_minor is null or p_amount_minor <= 0 then raise exception 'The price must be greater than zero.'; end if;
  if not exists (select 1 from public.examinations e where e.id = p_examination_id) then raise exception 'The examination was not found.'; end if;
  if p_effective_to is not null and p_effective_to <= coalesce(p_effective_from, now()) then raise exception 'The price expiry must be after its effective date.'; end if;
  ...
  insert into public.exam_prices (
    ...
    p_examination_id, v_currency, p_amount_minor, v_country_codes,
    p_is_default, p_is_active, coalesce(p_effective_from, now()), p_effective_to, auth.uid()
  )
  ...
  values (auth.uid(), 'upsert_exam_price', 'exam_price', v_price.id::text, jsonb_build_object(
...
declare
  ...
  v_currency text := case when p_currency is null or trim(p_currency) = '' then null else upper(trim(p_currency)) end;
  v_coupon public.coupons%rowtype;
begin
  if coalesce(public.current_user_role(), '') not in ('exam_admin', 'super_admin') then
    raise exception 'Only an examination administrator or Super Administrator may manage coupons.';
  end if;
  if v_code = '' or v_code !~ '^[A-Z0-9_-]{3,40}$' then raise exception 'Coupon codes must contain 3 to 40 letters, numbers, underscores or hyphens.'; end if;
  if p_discount_type not in ('percentage', 'fixed') then raise exception 'Discount type must be percentage or fixed.'; end if;
  if p_discount_value is null or p_discount_value <= 0 then raise exception 'Discount value must be greater than zero.'; end if;
  if p_discount_type = 'percentage' and p_discount_value > 100 then raise exception 'Percentage discounts cannot exceed 100 percent.'; end if;
  if v_currency is not null and v_currency !~ '^[A-Z]{3}$' then raise exception 'Coupon currency must be a valid three-letter ISO code.'; end if;
  if p_scope not in ('all', 'programme', 'examination') then raise exception 'Coupon scope is invalid.'; end if;
  if p_scope = 'all' and (p_programme_id is not null or p_examination_id is not null) then raise exception 'An all-examinations coupon cannot have a programme or examination target.'; end if;
  if p_scope = 'programme' and (p_programme_id is null or p_examination_id is not null) then raise exception 'A programme coupon requires exactly one programme target.'; end if;
  if p_scope = 'examination' and (p_examination_id is null or p_programme_id is not null) then raise exception 'An examination coupon requires exactly one examination target.'; end if;
  if p_expires_at is not null and p_starts_at is not null and p_expires_at <= p_starts_at then raise exception 'Coupon expiry must be after its start date.'; end if;
  if coalesce(p_per_candidate_limit, 0) < 1 then raise exception 'Per-candidate use limit must be at least one.'; end if;
  ...
  if p_coupon_id is null then
    insert into public.coupons (
      ...
      v_code, nullif(trim(coalesce(p_name, '')), ''), nullif(trim(coalesce(p_description, '')), ''),
      p_discount_type, p_discount_value, v_currency, p_scope,
      ...
      greatest(coalesce(p_minimum_amount_minor, 0), 0), p_maximum_discount_minor,
      p_starts_at, p_expires_at, p_maximum_redemptions,
      p_per_candidate_limit, p_is_active, auth.uid()
    ...
    returning * into v_coupon;
    if not found then raise exception 'The coupon was not found.'; end if;
  end if;

  insert into public.audit_logs (actor_id, action, entity_type, entity_id, metadata)
  values (auth.uid(), 'upsert_coupon', 'coupon', v_coupon.id::text, jsonb_build_object(
...
begin
  ...
  update public.coupons c set is_active = p_is_active, updated_at = now()
  where c.id = p_coupon_id returning * into v_coupon;
  if not found then raise exception 'The coupon was not found.'; end if;
  insert into public.audit_logs (actor_id, action, entity_type, entity_id, metadata)
  values (auth.uid(), 'set_coupon_active', 'coupon', v_coupon.id::text,
...
begin
  ...
  update public.exam_prices ep set
    is_active = p_is_active,
    is_default = case when p_is_active then ep.is_default else false end,
    updated_at = now()
  where ep.id = p_price_id returning * into v_price;
  if not found then raise exception 'The price was not found.'; end if;
  insert into public.audit_logs (actor_id, action, entity_type, entity_id, metadata)
  values (auth.uid(), 'set_exam_price_active', 'exam_price', v_price.id::text, jsonb_build_object(
    'examination_id', v_price.examination_id,
    'currency', v_price.currency,
    'is_active', v_price.is_active
  ));
...
begin
  ...
  select eo.* into v_order from public.exam_orders eo where eo.id = p_order_id for update;
  if not found then raise exception 'The order was not found.'; end if;
  if v_order.status <> 'pending' then raise exception 'Only pending unpaid orders may be cancelled.'; end if;

  update public.exam_orders eo set
    status = 'cancelled',
    ...
    updated_at = now()
  where eo.id = v_order.id returning * into v_order;

  update public.coupon_redemptions cr set status = 'released', released_at = now(), updated_at = now()
  where cr.order_id = v_order.id and cr.status = 'reserved';
  update public.exam_payments pay set
    status = case when pay.status = 'initiated' then 'abandoned' else pay.status end,
    updated_at = now()
  where pay.order_id = v_order.id;

  insert into public.audit_logs (actor_id, action, entity_type, entity_id, metadata)
  values (auth.uid(), 'cancel_exam_order', 'exam_order', v_order.id::text, jsonb_build_object(
  ...
  return jsonb_build_object('orderId', v_order.id, 'reference', v_order.reference, 'status', v_order.status);
end;
$$;

-- Keep the compatibility layer callable only by trusted owner-side wrappers.
revoke all on function public.get_admin_commerce_snapshot(integer) from public, anon, authenticated;
revoke all on function public.admin_upsert_exam_price(uuid, text, bigint, text[], boolean, boolean, timestamptz, timestamptz) from public, anon, authenticated;
revoke all on function public.admin_upsert_coupon(uuid, text, text, text, text, numeric, text, text, uuid, uuid, bigint, bigint, timestamptz, timestamptz, integer, integer, boolean) from public, anon, authenticated;
revoke all on function public.admin_set_coupon_active(uuid, boolean) from public, anon, authenticated;
revoke all on function public.admin_set_exam_price_active(uuid, boolean) from public, anon, authenticated;
revoke all on function public.admin_cancel_exam_order(uuid, text) from public, anon, authenticated;
