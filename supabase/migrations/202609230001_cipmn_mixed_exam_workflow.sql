...
begin
 ...
 if not exists(select 1 from public.profiles where id=v_uid and role='candidate' and is_active) then raise exception 'Only an active candidate account may start an examination.'; end if;
 ...
 if not found then
   if not v_exam.allow_self_enrollment then raise exception 'You have not been assigned to this examination.'; end if;
   insert into public.exam_assignments(examination_id,candidate_id,status,available_from) values(p_examination_id,v_uid,'assigned',now()) returning * into v_assignment;
 end if;
 ...
 if not found then
   select count(*) into v_count from public.attempts where examination_id=p_examination_id and candidate_id=v_uid;
   v_max:=coalesce(v_assignment.max_attempts_override,v_exam.max_attempts);
   if v_count>=v_max then raise exception 'The maximum number of attempts has been reached.'; end if;
   v_exp:=now()+make_interval(mins=>v_exam.duration_minutes);
   if v_exam.ends_at is not null then v_exp:=least(v_exp,v_exam.ends_at); end if;
   if v_assignment.expires_at is not null then v_exp:=least(v_exp,v_assignment.expires_at); end if;
   insert into public.exam_sessions(assignment_id,examination_id,candidate_id,expires_at,client_fingerprint,current_section)
   values(v_assignment.id,p_examination_id,v_uid,v_exp,coalesce(p_client_fingerprint,'{}'::jsonb),'mcq') returning * into v_session;
 end if;
 select jsonb_build_object(
   'id',e.id,'title',e.title,'course',p.code,'durationMinutes',greatest(1,ceil(extract(epoch from(v_session.expires_at-now()))/60.0)::integer),
   'questionCount',(select count(*) from public.questions q where q.examination_id=e.id and q.is_active),'description',coalesce(p.description,e.instructions,''),
   'sessionId',v_session.id,'expiresAt',v_session.expires_at,'assignmentId',v_assignment.id,'examFormat',e.exam_format,'currentSection',v_session.current_section,'mcqScore',v_session.mcq_percentage,
   'mcqCount',(select count(*) from public.questions q where q.examination_id=e.id and q.is_active and q.section='mcq'),
   'theoryCount',(select count(*) from public.questions q where q.examination_id=e.id and q.is_active and q.section='theory'),
   'questions',coalesce((select jsonb_agg(jsonb_build_object('id',q.id,'text',q.question_text,'type',case when q.question_type='theory' then 'theory' else 'mcq' end,'section',q.section,
     'options',coalesce((select jsonb_agg(qo.option_text order by qo.position) from public.question_options qo where qo.question_id=q.id),'[]'::jsonb)) order by q.position)
