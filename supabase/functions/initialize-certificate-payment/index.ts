...
import { agileCertCertificatePaystackRequest } from '../_shared/agilecertCertificatePaystack.ts';
import {
  adminClient,
...
Deno.serve(async (request: Request) => {
  ...
  try {
    ...
    if (!productCode || !['achievement', 'professional'].includes(productCode)) {
      return jsonResponse(request, { error: 'Select a valid certificate product.' }, 400);
    }
    ...
    if (!orderResult || typeof orderResult !== 'object') {
      throw new Error('The certificate order was not returned.');
    }

    const orderSummary = orderResult as Record<string, unknown>;
    if (orderSummary.paymentRequired === false) {
    ...
    if (!orderId) throw new Error('The pending certificate order identifier is missing.');
    ...
    if (!Number.isSafeInteger(Number(order.payable_amount_minor)) || Number(order.payable_amount_minor) <= 0) {
    ...
    const paystackPayload = await agileCertCertificatePaystackRequest('/transaction/initialize', {
      method: 'POST',
      body: JSON.stringify({
        email: user.email,
        ...
        metadata: {
          flow: 'agilecert_certificate_payment',
          orderId: order.id,
    ...
    if (!authorizationUrl || !accessCode) {
      throw new Error('Paystack did not return a certificate checkout URL.');
    }
    ...
    const { error: paymentError } = await admin
      .from('agilecert_certificate_payments')
      .upsert(
        {
          order_id: order.id,
          provider: 'paystack',
