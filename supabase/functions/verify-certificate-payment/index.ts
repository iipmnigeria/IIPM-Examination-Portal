  ...
  try {
    ...
    if (['paid', 'waived'].includes(order.status) && order.fulfilled_at) {
      const { data: fulfilment, error: fulfilmentError } = await admin.rpc(
        'agilecert_issue_certificate_for_order',
        {
          p_order_id: order.id,
          p_actor_id: null,
          p_authorisation_type: order.status === 'waived' ? 'administrator_waiver' : 'verified_payment',
        },
      );
    ...
    if (transactionReference !== order.reference) {
      throw new Error('The Paystack certificate transaction reference does not match the order.');
    }
    if (transactionCurrency !== order.currency.toUpperCase()) {
      throw new Error('The Paystack certificate transaction currency does not match the order.');
    }
    if (requestedAmount !== Number(order.payable_amount_minor)) {
      throw new Error('The Paystack certificate transaction amount does not match the order.');
    }
    if (transactionEmail && user.email && transactionEmail !== user.email.toLowerCase()) {
      throw new Error('The Paystack certificate customer email does not match the signed-in candidate.');
    }
