...
const App: React.FC = () => {
  ...
  const handleStartExam = async (testId: string) => {
    ...
    try {
      ...
      // D2: the secure-start payload intentionally refreshes session-sensitive data,
      // but older secure-start database functions do not echo the catalogue exam format.
      // Preserve only the server-catalogue format metadata so a CIPMN mixed assessment
      // stays on its existing MCQ -> Theory runtime instead of falling into the generic
      // one-stage completion path. Do not merge questions or any session/security data.
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans antialiased flex flex-col">
      ...
      {portalError && view !== 'exam' && (
        <div className="max-w-7xl w-full mx-auto px-4 pt-4">
          <div className="rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm font-semibold text-red-700">{portalError}</div>
        </div>
      )}

      <main className={view === 'exam' ? 'flex-1' : 'flex-1'}>
        {view === 'dashboard' && (
          <StudentDashboard
            ...
            onDismissCompletedAttempt={() => setJustCompletedAttempt(null)}
          />
        )}
        {view === 'admin' && <AdminDashboard attempts={attempts} tests={tests} onViewAttemptDetails={handleViewAttemptDetails} />}
        ...
        {view === 'exam' && selectedTest && (
          selectedTest.examFormat === 'cipmn_mixed' ? (
            <CipmnExamExperience
              ...
              onExit={() => { setSelectedTest(null); setView('dashboard'); void fetchPortalData(); }}
            />
          ) : (
            <ExamExperience
              test={selectedTest}
