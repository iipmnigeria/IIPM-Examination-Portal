...
export default function AdminPortal({ attempts, onBackToDashboard, onRefresh }: AdminPortalProps) {
  ...
  const canFlag = Boolean(summary && summary.eventCount > 0 && summary.evidenceRiskScore >= 25);
  const canInvalidate = Boolean(summary
    && summary.evidenceRiskScore >= 60
    && (summary.snapshotCount > 0 || summary.highEventCount >= 2));
                ...
                {loadingEvidence ? (
                  ...
                  <>
                    ...
                    <div className="mt-4 flex flex-wrap gap-2 rounded-xl border border-slate-200 bg-slate-50 p-3">
                      ...
                      <button disabled={busy || !canFlag} onClick={() => void runDecision('flag_attempt', 'State the specific persisted evidence supporting this flag:')} className="rounded-lg bg-amber-600 px-3 py-2 text-xs font-black text-white disabled:opacity-40">Flag with evidence</button>
                      <button disabled={busy || !canInvalidate} onClick={() => void runDecision('invalidate_attempt', 'State the evidence supporting invalidation of this attempt:')} className="rounded-lg bg-rose-700 px-3 py-2 text-xs font-black text-white disabled:opacity-40">Invalidate attempt</button>
                    </div>
                  </>
                )}
              </section>
