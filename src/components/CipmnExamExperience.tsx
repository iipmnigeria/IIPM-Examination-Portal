...
export default function CipmnExamExperience(props: Props) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (mediaGuardInstalled && mediaDevices && originalGetUserMedia) mediaDevices.getUserMedia = originalGetUserMedia;
    ...
    return (
      <div className="min-h-screen bg-slate-950 p-6 text-white flex items-center justify-center">
        <div className="w-full max-w-xl rounded-3xl border border-slate-800 bg-slate-900 p-8 text-center shadow-2xl">
          ...
          <p className="mt-3 text-sm leading-6 text-slate-300">Camera and AI processing are being restricted to the permissions allowed by the active examination policy.</p>
          {runtimeError && <div className="mt-5 rounded-2xl border border-rose-800 bg-rose-950/40 p-4 text-sm font-bold text-rose-200">{runtimeError}</div>}
          {runtimeError && <button type="button" onClick={props.onExitExam} className="mt-5 rounded-xl border border-slate-700 px-5 py-3 text-sm font-black text-slate-200 hover:bg-slate-800">Return to dashboard</button>}
        </div>
      </div>
    );
  }
