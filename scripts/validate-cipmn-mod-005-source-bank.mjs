...
const path = 'supabase/migrations/20260926010000_cipmn_mod_005_source_question_bank.sql';
const sql = fs.readFileSync(path, 'utf8');

const fail = (message) => {
  console.error('[cipmn-mod-005-source-bank] ' + message);
  process.exitCode = 1;
};
...
function normalizedTokens(text) {
  const stop = new Set(['a','an','the','and','or','to','of','in','on','for','with','which','is','are','from','this','that','most','module','project','risk','risks','issue','issues']);
...
for (const item of mcqs) {
  ...
  correctPositions.add(item.correct);
}
expect(correctPositions.size >= 3, 'Correct answers must not be concentrated in a single option position.');
...
for (const required of [
  'opportunity','threat','brainstorming','SWOT','Delphi','assumption analysis','Risk Breakdown Structure',
  'probability-impact','urgency','qualitative','quantitative','EMV','Decision Tree','Monte Carlo',
  'Sensitivity Analysis','Avoid','Mitigate','Transfer','Accept','Exploit','Enhance','Share',
  'Residual Risk','Secondary Risk','contingency','fallback','Risk Register','Issues Log',
  'escalation','Risk Radar','communication','stakeholder','lessons learned'
]) {
  expect(joined.toLowerCase().includes(required.toLowerCase()), 'Required official-source topic is not assessed: ' + required);
}

for (const forbidden of [
  'value at risk','var calculation','beta distribution','pert distribution','black swan index',
  'bow-tie analysis','fmea','hazop','coso erm','risk-adjusted net present value',
  'cost performance index','schedule performance index'
]) {
  expect(!joined.toLowerCase().includes(forbidden), 'Question bank includes material not supported by the official Module 5 sources: ' + forbidden);
}

expect(sql.includes("status='active'") && sql.includes('expires_at>now()'), 'Migration must block while a genuinely live session exists.');
expect(sql.includes('position=position+11000'), 'Legacy questions must be retired out of active positions.');
...
expect(sql.includes('CIPMN-MOD-005 official PDF + PPTX; reviewed 2026-09-25'), 'Theory rubrics must identify both official Module 5 sources.');

if (!process.exitCode) {
  console.log('[cipmn-mod-005-source-bank] Official-source scope, uniqueness, difficulty structure and migration safety checks passed.');
}
