...
const path='supabase/migrations/20260926063000_cipmn_mod_010_pptx_question_bank.sql';
const sql=fs.readFileSync(path,'utf8');
const fail=(m)=>{console.error('[cipmn-mod-010-pptx-bank] '+m);process.exitCode=1;};
...
function tokens(text){
  const stop=new Set(['a','an','the','and','or','to','of','in','on','for','with','which','is','are','from','this','that','most','module','project','leadership','team','teams']);
...
for(const required of [
  'project management','project leadership','pmi talent triangle','vuca',
  'autocratic','democratic','transformational','servant','laissez-faire','situational',
  'sustainable leadership','sdg','stakeholder matrix','balanced scorecard','scenario planning',
  'psychological safety','tuckman','forming','storming','norming','performing','adjourning',
  'raci','self-awareness','self-regulation','motivation','empathy','social skills',
  'sbi','continuous alignment','recognition','stress management','high-performance wheel',
  'purpose','people','processes','performance','partnership',
  'micromanagement','adaptive leadership','coaching mindset','ai integration',
  'remote','hybrid','cultural intelligence','lagos–calabar'
]){
  expect(joined.includes(required),'Required deck topic not assessed: '+required);
}

for(const forbidden of [
  'hersey blanchard maturity levels','goleman six leadership styles',
  'eq-i 2.0','msceit','disc personality model','belbin team roles',
  'lencioni five dysfunctions','drexler sibbet','grpi model',
  'servant leadership ten principles','balanced scorecard strategy map formula',
  'earned value formula','cpi','spi'
]){
  expect(!joined.includes(forbidden),'Unsupported outside/deeper leadership content found: '+forbidden);
}

expect(sql.includes("status='active'")&&sql.includes('expires_at>now()'),'Migration must block on genuinely live sessions.');
expect(sql.includes('position=position+21000'),'Legacy questions must be retired out of active positions.');
...
expect(sql.includes('CIPMN-MOD-010.pptx (107 slides; reviewed 2026-09-26)'),'Theory rubrics must identify the sole official Module 10 source.');

if(!process.exitCode) console.log('[cipmn-mod-010-pptx-bank] Deck scope, uniqueness, answer diversity and migration safety checks passed.');
