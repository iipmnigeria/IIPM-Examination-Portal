...
select
  pl.candidate_id,
  lower(pl.email),
  encode(extensions.digest(lower(pl.email), 'sha256'), 'hex'),
  pl.message_type,
  case
    when pl.message_type in ('cipmn_payment_recovery','cipmn_mock_resume','cipmn_mock_start')
      then 'operational'
    else 'marketing'
  end,
  'cipmn-oct-2026:' || pl.message_type || ':' || pl.candidate_id::text || ':' ||
    to_char(((select as_of from params) at time zone 'Africa/Lagos')::date, 'YYYYMMDD') ||
    ':slot-' || pl.send_slot::text,
  (select as_of from params) + case when pl.send_slot = 1 then interval '0 minutes' else interval '6 hours' end,
  jsonb_build_object(
    'campaign', 'cipmn-oct-2026',
    'moduleCodes', pl.module_codes,
    'moduleCode', pl.module_codes,
    'nearestExaminationDate', pl.nearest_examination_date,
    'nextExaminationDate', pl.nearest_examination_date,
    'examinationDate', pl.nearest_examination_date,
    'reference', pl.reference,
    'completedCount', pl.completed_count,
    'monitorCopyEmail', pl.monitor_copy_email
  )
from planned pl
join public.agilecert_communication_preferences pref
  on pref.candidate_id = pl.candidate_id
cross join control
where control.write_enabled = true
  and pl.eligible_for_existing_outbox
  and (
    pl.message_type in ('cipmn_payment_recovery','cipmn_mock_resume','cipmn_mock_start')
      and pref.operational_messages
    or pl.message_type in ('cipmn_unpurchased_modules','cipmn_exam_preparation')
      and pref.course_recommendations
  )
on conflict (event_key) do nothing
returning id,event_key,message_type,candidate_id,recipient_email,due_at;

-- SAFETY: write_enabled is hard-coded false above. This file is a staging artifact only.
